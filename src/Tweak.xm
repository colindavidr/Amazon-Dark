        ...
        @"(function(){try{"
         ...
         "window.__AMZDARK_FIXCONTRAST__=function(){try{"
           ...
           "var pr='';try{{"
             ...
             "}catch(e){}"
             "try{var KBQ=document.querySelectorAll("
               "'[aria-label*=More],[aria-label*=more],[aria-label*=ptions],[aria-label*=verflow],"
               "[class*=overflow],[class*=kebab],[class*=ellipsis],[class*=more-menu],[class*=meatball]');"
               "var kb=[];for(var kq=0;kq<KBQ.length&&kb.length<5;kq++){var ke=KBQ[kq];"
                 "var kr=ke.getBoundingClientRect();if(kr.width<8||kr.width>46||kr.height<8||kr.height>46)continue;"
                 "var kcs=getComputedStyle(ke),kbf=getComputedStyle(ke,'::before');"
                 "var kk='plain',kbi=kcs.backgroundImage,kmi=kcs.webkitMaskImage||kcs.maskImage;"
                 "if(ke.tagName.toLowerCase()==='img')kk='img';"
                 "else if(kmi&&kmi!=='none')kk='mask';"
                 "else if(kbi&&kbi!=='none')kk='bgimg';"
                 "else if(kbf.backgroundImage&&kbf.backgroundImage!=='none')kk='pre-bg';"
                 "else if(ke.namespaceURI==='http://www.w3.org/2000/svg')kk='svg';"
                 "var kc=ke.className;if(kc&&kc.baseVal!==undefined)kc=kc.baseVal;"
                 "kb.push(kk+'.'+String(kc||'').split(' ')[0].slice(0,20)"
                   "+'@'+Math.round(kr.width)+'x'+Math.round(kr.height)"
                   "+'/f:'+((kcs.filter&&kcs.filter!=='none')?'Y':'-'));}"
               "if(kb.length)window.__AD_KEBAB__=kb.join(' ~ ');"
             "}catch(e){}"
